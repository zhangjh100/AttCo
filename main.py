...
if __name__ == "__main__":
    ...
    for fold in range(5):
        ...
        # -------------------------- 4. 封装多卡模型 --------------------------
        print(arg.modelname)
        model = model.to(device)  # 先移到GPU
        if len(gpu_ids) > 1:
            # 用DataParallel包装模型，指定多卡
        ...
        if arg.pretrained is not None:
            ...
            # 提取state_dict（处理DataParallel的module.前缀）
            ...
            # 多卡模型需初始化module的权重（而非DataParallel容器）
        ...
        for epoch in range(arg.epochs):
            ...
            if min_loss > loss_val:
                ...
                if os.path.exists(filename):
                    os.remove(filename)
                    # 保存state_dict（推荐，而非整个模型），去掉module前缀
                if isinstance(model, torch.nn.DataParallel):
